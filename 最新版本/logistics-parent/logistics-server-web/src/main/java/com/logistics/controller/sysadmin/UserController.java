...
public class UserController {
	...
	public String toRole(String userId,HttpServletRequest request){
		request.setAttribute("user", userService.selById(userId));
		request.setAttribute("roleList",roleService.findAll());
		request.setAttribute("roleStr", userService.getRoleStr(userId));
	...
	public String role(String userId,@RequestParam("roleIds") List<String> list) {
		try {
			userService.updateRoleUsers(list, userId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "forward:/sysadmin/userAction_list";
